...
public class shengchengyuebao extends AppCompatActivity {
    ...
    public int findID(int beginyear, int beginmonth, int beginday, int nowyear, int nowmonth, int nowday) {
        ...
        if (cursor.moveToFirst()) {
            ...
            if (cursor_2.moveToFirst()) {
                ...
                nowday = datelist_2[2];
                benyuerenshu = findID(beginyear, beginmonth, beginday, nowyear, nowmonth, nowday);
            }
        } else{
    ...
    @SuppressLint("Range")
    public List<String> getJuzhuxiaoqu (int list[], SQLiteDatabase mrdb){
        ...
        while (id_first <= id_last) {
            ...
            if (cursor_2.moveToFirst()) {
                Log.d("getColumnindex", String.valueOf(cursor_2.getColumnIndex("居住小区")));//值为-1
                ...
                v.put("小区",tmp_2);
                mrdb.insert("monthreport",null,v);
                Log.d("tmp2", tmp_2);
            }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.shengchengyuebao_layout);
        //获取当前日期用作文件名
        Calendar calendar = Calendar.getInstance();//获取年月日
        String year = String.valueOf(calendar.get(Calendar.YEAR));
        int year_int = calendar.get(Calendar.YEAR);
        ...
        String name= year+month+day+hour+minute+"_monthly"+".db";
        Log.d("月报", name);
        monthreport= new reportmonthdatabase(this, name,null,1);
        SQLiteDatabase mrdb=monthreport.getWritableDatabase();
        int resultday;
        int resultmonth;
        int resultyear;
        if (day_int >= 30) {
            resultday = day_int - 30 + 1;
            resultmonth = month_int;
            resultyear = year_int;
        } else {
            if (month_int - 1 == 3 || month_int - 1 == 5 || month_int - 1 == 7 || month_int - 1 == 8 || month_int - 1 == 10 || month_int - 1 == 12) {
        ...
        int benyuerenshu = findID(resultyear, resultmonth, resultday, year_int, month_int, day_int);
        TextView xianshibenyuerenshu = (TextView) findViewById(R.id.xianshibenyuerenshu);
        Log.d("本周人数", String.valueOf(benyuerenshu));
        xianshibenyuerenshu.setText(String.valueOf(benyuerenshu));//OK

        int[] tmplist = findTwoID(resultyear, resultmonth, resultday, year_int, month_int, day_int);
        List<String> zhoubao = getJuzhuxiaoqu(tmplist,mrdb);
        Log.d("周报", String.valueOf(zhoubao));
        zhoubao.removeAll(Collections.singleton(null));//除去null值
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(shengchengyuebao.this, android.R.layout.simple_list_item_1, zhoubao);
        ListView listView_1 = (ListView) findViewById(R.id.yuebao);
        listView_1.setAdapter(adapter);

        if (ContextCompat.checkSelfPermission(shengchengyuebao.this, Manifest.permission.READ_EXTERNAL_STORAGE)
                != PackageManager.PERMISSION_GRANTED ||
                ContextCompat.checkSelfPermission(shengchengyuebao.this, Manifest.permission.WRITE_EXTERNAL_STORAGE)
                        != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(shengchengyuebao.this,
                    new String[]{Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE},
                    STORAGE_PERMISSION_REQUEST_CODE);
        } else {
            // 权限已经被授予，执行文件操作
            Log.d("perform week report", "OK");
            DatabaseHelper.copyDatabaseToPublicDirectory(shengchengyuebao.this, name);
        }
